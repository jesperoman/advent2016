...
//  println(rooms)
  ...
  def realRoom(code: String): Int = {
    code match {
      case pattern(f, sNr, c) =>
        val frequenceOrder = f.replaceAll("[^a-z]", "").sorted.groupBy(_.toInt).map {
          case (cc, letters) => letters.head -> letters.length
        }.toList.groupBy(_._2).toList.sortBy(_._1).reverse.map(_._2.map(_._1))

//        println(frequenceOrder)

        def loop(codeLeft: String, foLeft: List[List[Char]]): Int = {
          if (codeLeft.isEmpty) sNr.toInt
          else foLeft match {
            case Nil => 0
            case Nil :: others => loop(codeLeft, others)
            case head :: stuffLeft if head.contains(codeLeft.head) => loop(codeLeft.tail, head.filterNot(_ == codeLeft.head)::stuffLeft)
            case _ => 0
          }
        }

        loop(c, frequenceOrder)
    }
  }
  println(s"Answer 1: ${rooms.map(realRoom).sum}")

  val realRooms = rooms.map(r => r -> realRoom(r)).filter {
    case (_, secNr) => secNr > 0
  }

  val roomNames = realRooms.map{
    case (pattern(name, secNr, _), _) =>
      name.map(shiftLetter(secNr.toInt)) -> secNr
  }

  println(s"Answer 2: ${roomNames.filter{
    case (name, nr) => name.contains("north")
  }.head._2}")

  def shiftLetter(nr: Int)(char: Char): Char =
    if (char != '-') nr match {
      case 0 => char
      case x if char == 'z' => shiftLetter(nr - 1)('a')
      case x => shiftLetter(nr - 1)((char.toInt + 1).toChar)
    }
    else char

  lazy val input = """bkwzkqsxq-tovvilokx-nozvyiwoxd-172[fstek]
